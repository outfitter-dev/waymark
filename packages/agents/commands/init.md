...
Write `.waymark/config.yaml`:

```yaml
# Waymark configuration
# Generated by /waymark:init on [date]

strategy: minimal # or standard, comprehensive

markers:
  enabled: # Based on strategy
    - tldr
  custom: []
...
- If `.waymark/config.yaml` exists, ask whether to overwrite or merge
