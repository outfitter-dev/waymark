...
- **Signals** (optional): A short prefix indicating scope/urgency. The only valid signals are:
  - `*` — branch-scoped work that must be cleared before merging to protected branches.
  - `!` — high-priority item. When combined with `*`, the order is `*!` (e.g., `*!todo`). Double intensity (`!!`) and other legacy signals are not part of v1.
...
For long content use continuation lines:
...
Continuation lines start with `...` and the block closes on a line ending with `:::`. Avoid multi-line content when a concise single-line sentence will do.
...
rg '^\s*//\s*!\w+\s+:::'        # high-priority fixes/todos
rg '#perf:hotpath|#hotpath'        # performance hotspots
git ls-files '*.md' | xargs rg '<!-- tldr :::.*#docs'  # doc summaries
rg ':::\s*!tldr'                  # prioritized summaries
```
...
- Parsers should normalize signals to `*` and/or `!`, lowercase markers, trim extra spaces, and emit structured records matching the schema in `PRD.md`.
